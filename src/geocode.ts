...
function loadGroupData(group: string): Promise<string> {
  return new Promise(resolve => {
    if (group === currentGroup && currentGroupData) { // group hasn't changed and the data is already loaded
...
function prepareSearchIndex(group: string, numericWords: NumericWord[]): Promise<void> {
  return new Promise(resolve => {
    ...
    if (newSearchIndexKey === currentSearchIndexKey) { // no need to rebuild search index
      resolve();
      return;
    }
          ...
          if (block.u && numericWords.length > 0) {
            for (let unitType in block.u) {
              ...
              for (let unitNumber of unitNumbers) {
                if (Array.isArray(unitNumber) && unitNumber.length === 2) { // range of units - e.g. [1,8]
                ...
                else { // single unit number - e.g. 1 or "1A"
